...
export default function DashboardLayout({
  ...
  return (
    <div
      className={"flex h-screen bg-[#FDFDFD] transition-all duration-500 ease-in-out overflow-hidden"}
    >
      {/* Sidebar Component */}
      <aside className={`relative transition-all duration-300 h-full border-r border-gray-100 flex flex-col shrink-0 bg-white z-10 ${isSdbrClose ? 'w-65 sm:w-70' : 'w-20'}`}>
        <button onClick={() => setIsSdbrClose(!isSdbrClose)} className={`absolute text-black z-50 cursor-pointer top-5 ${isSdbrClose ? '-right-5' : '-right-5'}`}>
          {isSdbrClose ? <IoIosArrowDropleftCircle className="w-10 h-10" /> : <IoIosArrowDroprightCircle className="w-10 h-10" />}
        </button>
        {isSdbrClose ?
          <>
            ...
            <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
              <Link
                href="/dashboard"
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium transition-colors cursor-pointer ${isDashboardActive ? "bg-[#8A38F5] text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"
                  }`}
              >
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium transition-colors cursor-pointer ${isHistoryActive ? "bg-[#8A38F5] text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"
                  }`}
              >
              ...
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl font-medium transition-colors cursor-pointer ${isProfileActive ? "bg-[#8A38F5] text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"
                  }`}
              >
            ...
            <div className="p-4 border-t border-gray-100 mt-auto">
              ...
              >
                <img src="/images-dashboard/Logout.webp" alt="Logout" className="w-4.5 h-[18px] opacity-80" />
                Logout
              </button>
            </div>
          </>
          : (
            <>
              ...
              <nav className="flex-1 px-2 py-6 space-y-2 overflow-y-auto">
                <Link
                  href="/dashboard"
                  className={`flex items-center justify-center p-3 rounded-xl transition-colors ${isDashboardActive
                      ? "bg-[#8A38F5] shadow-sm"
                      : "hover:bg-gray-50"
                    }`}
                >
                ...
                <Link
                  href="/dashboard/history"
                  className={`flex items-center justify-center p-3 rounded-xl transition-colors ${isHistoryActive
                      ? "bg-[#8A38F5] shadow-sm"
                      : "hover:bg-gray-50"
                    }`}
                >
                ...
                <Link
                href="/dashboard/profile"
                className={`flex items-center justify-center p-3 rounded-xl transition-colors  ${isProfileActive ? "bg-[#8A38F5] text-white shadow-sm" : "text-gray-500 hover:bg-gray-50"
                  }`}
              >
                <img src="/images-dashboard/Profile.webp" alt="Profile" className={`w-5 h-5 ${isProfileActive ? "filter brightness-0 invert" : ""}`} />
              </Link>
              ...
              <button
                ...
                className="flex items-center gap-3 mt-4 px-3 text-[14px] text-gray-500 hover:text-gray-900 font-medium w-full text-left transition-colors"
              >
                <img src="/images-dashboard/Logout.webp" alt="Logout" className="w-4.5 h-4.5 opacity-80" />
              </button>
            </div>
            </>
          )}
