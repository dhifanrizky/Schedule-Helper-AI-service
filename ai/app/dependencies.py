...
async def get_graph() -> CompiledStateGraph:
    ...
    async with _graph_lock:
        ...
        counselor_llm = get_llm(
            provider=ROUTER_LLM_PROVIDER, model=ROUTER_LLM_MODEL, temperature=0.7
        )

        # 3. Prioritizer: Butuh logika urutan mantap
        prioritizer_llm = get_llm(
            provider=ROUTER_LLM_PROVIDER, model=ROUTER_LLM_MODEL, temperature=0.2
        )

        # 4. Scheduler: Butuh function calling konsisten
        scheduler_llm = get_llm(
